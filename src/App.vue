<template>
  <div id="app">
    <h2>1) Selecciona el tanque: </h2>
    <select v-model="selected_tank">
      <option disabled value="">Seleccione la capacidad:</option>
      <option>284</option>
      <option>379</option>
      <option>454</option>
      <option>511</option>
    ...
    <div v-if="selected_tank.length > 0">
      <h2>2) Selecciona la altura: </h2>
      <br/>
      <div>
        <vue-slider ref="slider" v-model="selected_index" min="0" max="31"></vue-slider>
      </div>
      <h2>{{ lookup }} litros.</h2>      
    </div>
  </div>
...
export default {
  ...
  computed: {
  lookup: function(){
    if (this.selected_tank != '' && this.selected_index != '') {
